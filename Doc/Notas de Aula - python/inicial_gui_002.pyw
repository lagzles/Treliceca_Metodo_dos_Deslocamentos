...
##        self.label_vazio4 = Label(self.container_direito, text='\t', height=1).grid(row=2, column=4)
    ...
    def vt_cobertura(self):
        ...
        if vt == 0:
            ...
            widget_1 = find_in_grid(self.container_direito, 2, 3)
            if widget_1 != None:
                widget_1.grid_forget()
    ...
    def create_vaos_label(self, n):    
        ...
        if self.viga_transicao.get() == 0:
            ...
            self.label_cumeeira = Label(self.container_esquerdo, text='Vão [m]', height=1).grid(row=1, column=1)
        ...
        if self.viga_transicao.get() == 0:
            ...
            for i in range(n):
                self.labe1 = Label(self.container_esquerdo, text='apoio {} [m]'.format(i+1), height=1).grid(row=(r+i), column=1)
                self.entry = Entry(self.container_esquerdo, width=5)
                self.entry.insert(0, '0')
                self.entry.grid(row=(r+i), column=2)
    ...
    def calcular_viga(self):
        ...
        comb_sc = - (cp*1.25 + sc * 1.5 + su * 1.4)
        comb_su = - (cp*1.25 + sc * 1.5 + su * 1.4)
        # combinação de vento
        comb_cv = - cp + pd * 1.4
        ...
        # retorno = [barras, nós]
        trelica_obj = geo.geometrizar(tempdir, n, lista_vaos, cumeeira, vao_secundaria, vento, viga_transicao)
        
        carga_grav = comb_grav * vao_secundaria
        carga_cv = comb_cv * vao_secundaria
        #print(vao_secundaria, "vao_secundaria / area de influencia?")
...
def mostrar_about_box():
    messagebox.showinfo("About Box", """Programa desenvolvido para automatizar
a verificação estrutural de vigas de cobertura ou vigas de transição treliçadas.
    """)
