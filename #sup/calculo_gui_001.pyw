...
##        vscrollbar = Scrollbar(self.container_baixo, orient=VERTICAL)
##        vscrollbar.pack(fill=Y, side=RIGHT, expand=FALSE)

##        self.canvas = Canvas(self.container_baixo,
##                             yscrollcommand=vscrollbar.set)
##        self.canvas.pack(side=LEFT, fill=BOTH, expand=TRUE)
        self.bars_canvas = Canvas(self.master)
...
##                                           anchor=NW)
        ...
        foo = []
        for b in barras:
            label1 = Label(interior, text=b.tipo, anchor=W)
            label1.grid(row=r, column=c)

            label2 = Label(interior, text=b.ni.id, anchor=W)
            label2.grid(row=r, column=c+1)
            
            label3 = Label(interior, text=b.nf.id, anchor=W)
