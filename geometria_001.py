...
def geometrizar(file, n_vaos, lista_vaos, cumeeira, pe_direito, tipo, trelica,
        ...
        for i in range(len(pontos_viga)-1):
            if tipo != 1: # se for desenho 3D
                ...
                if trelica == 1:
                    ...
                    trel.desenhar_trelica(dwg, h_viga, pontos_viga[i], pontos_viga[i+1], vento, primeiro_vao, pre_gdl, 
                                              (cumeeira, cumeeira_alinhada_com_pilar))
                    primeiro_vao = False
                else:
                ...
                if trelica == 1:
                    ...
                    h_viga = round(max(lista_vaos)/15,2)
                    if file != "":
                        trel.desenhar_trelica(dwg, h_viga, xi, xf, vento, primeiro_vao, pre_gdl,
                                              (cumeeira, cumeeira_alinhada_com_pilar))
                        print("desenhado")
                    if file == "":
                        retorno = trel.rigidez_trelica( h_viga, xi, xf, vento, primeiro_vao, pre_gdl, 
                                              (cumeeira, cumeeira_alinhada_com_pilar))
                        print("rigidez")
    ...
    print('fim')
    return (barras_objetos, nos_objetos)
